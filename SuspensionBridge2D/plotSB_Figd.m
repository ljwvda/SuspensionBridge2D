function plotSB_Figd(a_out,q,Nplot,Climit,v)
% plotSB creates a plot of the numerical approximation specifically for the
% case in Figure d (combination of two different wave patterns)
% Input:
...
a_outsym = [a_out flip(a_out(1:end,2:end),2); flip(a_out(2:end,1:end)) ...
    rot90(a_out(2:end,2:end),2)];
u = ifft2(a_outsym, 'symmetric')*numel(a_outsym);
u2sym = u(2:end,2:end);

% Shift the plot so we clearly see the combination of two type of solutions
u_aux1 = u2sym(ceil(size(u2sym,1)*3/4+1):end,:);
u_aux2 = u2sym(1:ceil(size(u2sym,1)*3/4),:);
u_shift = [u_aux1; u_aux2];
...
u2sym_interp = interp2(y1bold,y2bold,u_shift',Xb,Yb,'spline'); 
